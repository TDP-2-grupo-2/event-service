...
def test_given_a_new_event_when_an_organizer_wants_to_created_then_it_should_create_it():
    ...
    response = client.post("/organizers/event/", json=json_rock_music_event, headers={"Authorization": f"Bearer {token}"})
...
def test_given_a_date_that_passed_when_creating_an_event_then_it_should_not_create_it():
    ...
    response = client.post("/organizers/event/", json=json_event_with_invalid_date, headers={"Authorization": f"Bearer {token}"})
...
def test_WhenTheClientTriesToGetAllTheEvents_ThereAreThreeEvents_TheAppReturnsAListWithAllThreeEvents():
    token = jwt_handler.create_access_token("1", 'organizer')
    client.post("/organizers/event/", json=json_rock_music_event, headers={"Authorization": f"Bearer {token}"})
    client.post("/organizers/event/", json=json_theatre_event, headers={"Authorization": f"Bearer {token}"})
    client.post("/organizers/event/", json=json_programming_event, headers={"Authorization": f"Bearer {token}"})
...
def test_WhenTheClientTriesToGetEventsByManyTags_ManyMatch_TheAppReturnsTheEventsCorrectly():
    ...
    client.post("/organizers/event/", json=json_rock_music_event, headers={"Authorization": f"Bearer {token}"})
    client.post("/organizers/event/", json=json_theatre_event, headers={"Authorization": f"Bearer {token}"})
    client.post("/organizers/event/", json=json_lollapalooza_first_date, headers={"Authorization": f"Bearer {token}"})
...
def test_WhenTheClientReservesAnExistingEvent_ThenItShouldIncreaseitAttendase():
    token = jwt_handler.create_access_token("1", 'organizer')
    response = client.post("/organizers/event/", json=json_programming_event, headers={"Authorization": f"Bearer {token}"})
